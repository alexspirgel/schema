...
const Schema = class {
	...
	validate(input, model) {
		...
		if(!model) {
			// Use the class instance model as the model.
			model = this.model;
		}
		...
		// If the model consists of a single schema object.
			...
			if(typeof input === 'undefined' || input === null) {
				...
				if(model.required) {
					console.error('Schema Validation Error: _____ is a required value.');
					return false;
				}
				...
				// If the type option value is object.
					...
					if(model.object_property_schema) {
						...
						for(let schema_property in model.object_property_schema) {
							...
							const object_property_is_valid = this.validate(input[schema_property], model.object_property_schema[schema_property]);
